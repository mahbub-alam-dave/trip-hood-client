...
const BookingForm = ({ packageData, guides }) => {
  ...
  // three time bookings congratulations
  ...
  // get bookings (previously done by user)
  const { data: userBookings = [] } = useQuery({
    queryKey: ["all-packages"],
    queryFn: async () => {
      const res = await axiosSecure.get(
        `${import.meta.env.VITE_app_url}/bookings?email=${user?.email}`
      );
      return res.data;
    },
  });
  ...
  const bookingMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // show congratulations animation when user book more than three packages
      if(userBookings.length >= 3) {
        // alert("congratulations! You have trusted more than three times")
  ...
  return (
    <div className="space-y-7">
      <div className=" space-y-2">
        <h2 className="text-2xl font-bold text-[var(--color-text-secondary)] dark:text-[var(--color-text-secondary-dark)]">
          Book This Tour
        </h2>
        <p className="text-[var(--color-text-secondary)] dark:text-[var(--color-text-secondary-dark)] text-sm">
          Fill the details to reserve your seat.
        </p>
      </div>
      <div className=" grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="space-y-4 px-6 bg-[var(--color-bg-primary)] dark:bg-[var(--color-bg-primary-dark)] py-12 rounded-xl shadow-lg"
        >
          {/* Tourist Photo */}
          ...
          {/* Tourist Name */}
          <div>
            <label className="text-sm font-medium text-[var(--color-text-secondary)] dark:text-[var(--color-text-secondary-dark)]">
              Tourist Name
            </label>
            <input
              type="text"
          ...
          <div>
            ...
            <select
              ...
              defaultValue=""
            >
              <option className="dark:bg-gray-600" value="" disabled>
                Select a guide
              </option>
              {guides.map((guide) => (
                <option
                  key={guide._id}
                  className="hover:bg-[var(--color-primary)] dark:bg-gray-600"
